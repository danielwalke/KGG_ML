...
for fold_idx, (train_index, test_index) in enumerate(outer_cv.split(X, y), 1):
    ...
    def get_best_params(X_train, y_train):
        def objective(params):
            clf = GNNModel(ontology_list, num_classes, params["lr"], params["C"], int(params["epochs"]), params["dropout"], device=device)
    ...
    model = GNNModel(ontology_list, num_classes, best_params["lr"], best_params["C"], int(best_params["epochs"]), best_params["dropout"], device=device) # C= 10, lr = 1e-2, dropout = 0, epochs, 500
