...
protein_emd = np.load("../embeddings.npy")
...
direct_edge_index = torch.stack([torch.arange(tax_edges.shape[0]), torch.arange(tax_edges.shape[0])])
X = data_df.values
direct_layer = OnotologyLayer("protein", "protein", data_df.values.shape[-1], direct_edge_index)
tax_ontology = Ontology("Taxonomy")
tax_ontology.add_layer(direct_layer)
...
for i in tqdm(range(10)):
    set_all_seeds(i)
    model = GNNModel(ontology_list, num_classes, best_params["lr"], best_params["C"], 500, best_params["dropout"], device=device, protein_embeddings = protein_emd)
